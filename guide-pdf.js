...
async function downloadGuidePDF() {
  ...
  try {
    ...
    h1('7 — Scheda Backtest Storico (1970-2025)');
    p('Il tab Backtest Storico applica il piano configurato nel Simulatore (portafoglio, PAC, orizzonte, TER) alla serie mensile 1970-2025 (672 rendimenti mensili reali degli indici in EUR — non ricostruzioni), partendo da 10 periodi storici chiave. Risponde alla domanda: come si sarebbe comportato questo piano nel passato?');
    ...
    p('Sezione aggiuntiva del tab Backtesting: simulazione del percorso mensile preciso del portafoglio durante le 10 principali crisi macro della storia moderna (1970-2024), con le serie mensili reali degli indici in EUR: ogni singolo mese (incluso ott 1987, ott 2008, mar 2020) è il dato storico vero, non una ricostruzione. Come nel Rischio di Sequenza, puoi scegliere la modalita di versamento (capitale + PAC, solo capitale, solo PAC) e la fase del piano in cui arriva la crisi (inizio, meta, fine): cosi il capitale esposto al crollo riflette quello realmente accumulato a quel punto del piano.');
    ...
    li('Fonte dati: 672 rendimenti mensili REALI dal 1970 al 2025. Azioni sviluppate (MSCI World Net EUR), USA, Europa, oro e mercati emergenti sono le serie mensili vere degli indici in EUR, mese per mese. Serie reali dedicate anche per: governativo globale hedged (dal 1985), obbligazioni euro aggregate (microstruttura reale dal 2009), inflation-linked (dal 2005), aggregato globale hedged (dal 2018) e commodities Bloomberg (dal 2005), ciascuna con fallback dichiarato prima della propria data di inizio. Fattori azionari e Bassa Volatilita usano dati reali Kenneth French/AQR (dal 1979), i REITs la serie FTSE Nareit in EUR (dal 1979). L\'inflazione per i valori reali e quella EURO: CPI Germania fino al 1998 (coerente con la convenzione DEM delle serie pre-euro) e area euro dal 1999 — non il CPI USA.');
    ...
    p('Matrice di correlazione calibrata su dati storici 1970-2025 (30x30 asset class, con correlazioni base categoria-categoria che coprono tutte le coppie). Rendimenti attesi selezionabili tra forward-looking (CAPE-adjusted, conservativi) e storici (CAGR 1970-2025). Usare come guida qualitativa — non come prescrizione assoluta (instabilita di Markowitz).');
